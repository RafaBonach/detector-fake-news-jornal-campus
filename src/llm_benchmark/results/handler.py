...
class Store:
    ...
    def save_results(self, results: pd.DataFrame):
        ...
        self.__results_dataframe__ = pd.concat([self.__results_dataframe__, results.convert_dtypes()], ignore_index=False)
        
        self.__results_dataframe__.to_csv(self.path, index=False)
        print(f"Resultados salvos em {self.path}")

    def get_results(self) -> pd.DataFrame:
        """Retorna o DataFrame com os resultados armazenados."""
        return self.__results_dataframe__
    '''
